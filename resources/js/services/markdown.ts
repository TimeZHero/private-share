...
export function computeMarkdownInsertion(
    ...
    switch (type) {
        case 'bold':
            ...
            if (!selectedText) selectRange = [selectionStart + 2, selectionStart + 11];
            break;
        case 'italic':
            insertion = `*${selectedText || 'italic text'}*`;
            if (!selectedText) selectRange = [selectionStart + 1, selectionStart + 12];
            break;
        case 'strikethrough':
            insertion = `~~${selectedText || 'strikethrough'}~~`;
            if (!selectedText) selectRange = [selectionStart + 2, selectionStart + 15];
            break;
        case 'heading': {
            const lineStart = text.lastIndexOf('\n', selectionStart - 1) + 1;
            const prefix = selectionStart === lineStart ? '' : '\n';
            insertion = `${prefix}## ${selectedText || 'Heading'}`;
            if (!selectedText) selectRange = [selectionStart + prefix.length + 3, selectionStart + prefix.length + 10];
            break;
        }
        case 'link':
            if (selectedText) {
                insertion = `[${selectedText}](url)`;
                selectRange = [selectionStart + selectedText.length + 3, selectionStart + selectedText.length + 6];
            } else {
        ...
        case 'image':
            ...
            if (!selectedText) selectRange = [selectionStart + 2, selectionStart + 10];
            break;
        case 'code':
            insertion = `\`${selectedText || 'code'}\``;
            if (!selectedText) selectRange = [selectionStart + 1, selectionStart + 5];
            break;
        case 'codeblock': {
            const lang = 'language';
            insertion = `\n\`\`\`${lang}\n${selectedText || 'code here'}\n\`\`\`\n`;
            if (!selectedText) selectRange = [selectionStart + 5 + lang.length, selectionStart + 5 + lang.length + 9];
            break;
        }
        case 'quote':
            insertion = `\n> ${selectedText || 'Quote text here'}\n`;
            if (!selectedText) selectRange = [selectionStart + 3, selectionStart + 18];
            break;
        case 'bullet':
            insertion = `\n- ${selectedText || 'List item'}\n- \n- \n`;
            if (!selectedText) selectRange = [selectionStart + 3, selectionStart + 12];
            break;
        case 'numbered':
            insertion = `\n1. ${selectedText || 'First item'}\n2. \n3. \n`;
            if (!selectedText) selectRange = [selectionStart + 4, selectionStart + 14];
            break;
        case 'checkbox':
            insertion = `\n- [ ] ${selectedText || 'Task item'}\n- [ ] \n- [ ] \n`;
            if (!selectedText) selectRange = [selectionStart + 7, selectionStart + 16];
            break;
        case 'table':
    ...
    const cursorPosition = selectRange ? null : selectionStart + insertion.length - cursorOffset;
...
export const TOOLBAR_GROUPS: ToolbarAction[][] = [
    [
        { type: 'heading', label: 'Heading', shortcut: 'Ctrl+H', icon: 'heading' },
